...
// Get featured products (only from active vendors)
$featured_products_query = "
    SELECT p.id, p.name, p.price, p.compare_price, p.image_url, p.vendor_id, p.stock_quantity,
...
$featured_products = $database->fetchAll($featured_products_query);
...
$categories_query = "
    ...
    LIMIT 8
";
$categories = $database->fetchAll($categories_query);

// Get platform statistics
$stats = [
    'total_products' => $database->fetch("SELECT COUNT(*) as count FROM products WHERE status = 'active'")['count'],
    'total_vendors' => $database->fetch("SELECT COUNT(*) as count FROM users WHERE user_type = 'vendor' AND status = 'active'")['count'],
    'total_orders' => $database->fetch("SELECT COUNT(*) as count FROM orders WHERE payment_status = 'paid'")['count'],
    'total_customers' => $database->fetch("SELECT COUNT(*) as count FROM users WHERE user_type = 'customer' AND status = 'active'")['count']
];
        ...
        <div class="hero-content">
            <div class="hero-text">
                ...
                <div class="hero-stats">
                    ...
                    <div class="stat-item">
                        <span class="stat-number"><?php echo number_format($stats['total_vendors']); ?>+</span>
                ...
                <div class="card product-card">
                    ...
                    <div class="product-info">
                        ...
                        <div style="margin-top: 1rem;">
                            <a href="product.php?id=<?php echo $product['id']; ?>" class="btn btn-primary btn-sm" style="width: 100%;">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
...
<section style="padding: 4rem 0; background: var(--light-gray);">
    <div class="container">
        <h2 class="text-center mb-3">Why Choose MarketHub?</h2>
        <div class="row">
            <div class="col-4 text-center mb-3">
                <div style="width: 80px; height: 80px; background: var(--primary-green); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; color: white; font-size: 2rem;">🛍️</div>
                <h5>Wide Selection</h5>
                <p>Browse products from multiple verified vendors in one convenient location.</p>
            </div>
            <div class="col-4 text-center mb-3">
                <div style="width: 80px; height: 80px; background: var(--primary-green); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; color: white; font-size: 2rem;">⚖️</div>
                <h5>Easy Comparison</h5>
                <p>Compare prices, quality, and reviews across different vendors to make informed decisions.</p>
            </div>
            <div class="col-4 text-center mb-3">
                <div style="width: 80px; height: 80px; background: var(--primary-green); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; color: white; font-size: 2rem;">🔒</div>
...
<section style="padding: 4rem 0; background: var(--primary-green); color: white;">
    <div class="container text-center">
        <h2 style="color: white; margin-bottom: 1rem;">Ready to Start Selling?</h2>
        <p style="font-size: 1.2rem; margin-bottom: 2rem; color: white;">Join hundreds of vendors already selling on MarketHub</p>
        <a href="vendor/register.php" class="btn btn-secondary btn-lg">Register as Vendor</a>
    </div>
</section>
